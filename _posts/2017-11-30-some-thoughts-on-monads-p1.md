...
where `map` is a function that does a "big upgrade": it upgrades a function of type `A -> A` to a function of type `M -> M`. **[1]**
...
Look into `map id` and re-read the definition of the `map` function at **[1]**. Logically, this expression should produce a function that is also an identity one. In other words: `map id = id`. So we have:
...
## Do Notation or Computation Expressions

The three monad laws are quiet important for a wonderful language feature: the so-called *Do Notation* in Haskell, or *Computation Expressions* in F#. These provide very nice sugar-syntax when composing functions with monads.
